        ...
        if (pinStat(pinsSW[b], bcdSW[b]))
        {
            ...
            if (pinStat(pinsSW[b], bcdSW[b]))
            {
                if (b == 0) //NMI
                {
                    ...
                    {
                        // Control+Alt+F5
                        pulsafn(Y_P_ROW, Y_COL, codeset == 1 ? KS1_F5 : KEY_F5, 0, 0, 1, 1, 5);
                    }
                }    
                if (b == 1) //VGA-SWICH
                {
                    if (modo == zx || modo == at8 || modo == bbc || modo == vic || modo == ori)
                    {
                        pulsafn(A_G_ROW, G_COL, codeset == 1 ? KS1_SCRLCK : KEY_SCRLCK, 0, 0, 1, 1, 5);
                    }
                    if (modo == msx)
                    {
